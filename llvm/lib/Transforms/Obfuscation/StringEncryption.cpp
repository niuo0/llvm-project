...
struct StringEncryption : public ModulePass {
    ...
    string obfcharacters="0123456789ABCDEF";
    int seed;
    string randomString(int length = 7){
        string name = "unk_10";
        int t = length + name.size();
        name.resize(t);
        srand(seed);
        seed++;
        for(int i = t-length; i < t; i++){
...
__check:
        ...
        for (GlobalVariable *GV : rawStrings) {
            ...
            GlobalVariable *EncryptedRawGV = new GlobalVariable(
                                                                *(GV->getParent()), EncryptedConst->getType(), false,
                                                                GV->getLinkage(), EncryptedConst, randomString(), nullptr,
        ...
        for (GlobalVariable *GV : objCStrings) {
            ...
            GlobalVariable *EncryptedOCGV = new GlobalVariable(
                                                               *(GV->getParent()), newCS->getType(), false, GV->getLinkage(), newCS,
                                                               randomString(), nullptr, GV->getThreadLocalMode(),
        ...
        // Replace Uses
        for (User *U : Users) {
            //errs() << "user" << U->stripPointerCastsAndAliases()->getName() << "\n";
            for (map<GlobalVariable *, GlobalVariable *>::iterator iter =
                 old2new.begin();
                 iter != old2new.end(); ++iter) {
                
//                if (iter->first->hasOneUser()) {
//                    User * u = dyn_cast<User>(*iter->first->user_begin());
//                    errs() << "replace" << u->stripPointerCastsAndAliases()->getName() << "\n";
//                }
...
//                    errs() << "array" << gv->getName();
//                    if (gv == iter->first) {
//                        errs() << "array eq" << gv->getName();
...
//                if (iter->first->hasOneUser()) {
//                    User * u = dyn_cast<User>(*iter->first->user_begin());
//                    errs() << "clean" << u->stripPointerCastsAndAliases()->getName() << "\n";
//                }
//            }
        }
