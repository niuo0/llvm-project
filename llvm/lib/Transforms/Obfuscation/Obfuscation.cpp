...
static cl::opt<bool> EnableMBAObfuscation("mba", cl::init(false), cl::desc("Enable the MBAObfuscation pass"));
...
//        delete MP;
        ...
        FunctionPass *P = createMBAObfuscationPass(EnableAllObfuscation || EnableMBAObfuscation);
        for (Module::iterator iter = M.begin(); iter != M.end(); iter++) {
            Function &F = *iter;
            if (!F.isDeclaration()) {
                P->runOnFunction(F);
